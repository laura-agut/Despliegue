...
Pero supongamos que queremos que otro usuario (por ejemplo nuestro profesor) pueda acceder a la máquina con permisos de administrador. Para ello deberíamos crearle un usuario `profe` y darle permisos de `sudo` . Estos permisos nos permitirán que cualquier comando que ejecutemos en el terminal precedido de la palabra `sudo` se ejecute como *root*. De la misma forma, cualquier comando que ejecutemos con nuestro usuario sin `sudo`, será ejecutado con los permisos de nuestro usuario, por lo que nos protegemos de *liarla* con un comando que no toca como *root*.

Dicho esto, hay varias formas de proceder, veamos una sencilla paso a paso. 
...
Luego, agrega al nuevo usuario "profe" al grupo "sudo" para otorgarle permisos de administrador:
...
Pero ahora todavía no podemos conectarnos a la máquina con el usuario profe, porque solo podemos conectarnos con un par de claves y `profe` no las tiene. Vamos a generárselas.


## Generación del par de claves.

En primer lugar nos crearemos el par de claves para `profe`, pública y privada. Para ello abre una segunda terminal y en esta NO te conectes a AWS. **Desde esta terminal del ordenador local**, utiliza el comando (**sin sudo**):
...
Este proceso de copia se puede realizar fácilmente con el comando `ssh-copy-id` si nuestro servidor aceptara login por contraseña, pero como nuestro servidor solo acepta login por claves ssh, entonces deberemos hacerlo manualmente. Así entenderemos mejor lo que ocurre realmente. Sigue estos pasos:
...
Si pbcopy no funciona puedes hacer cat y simplemente seleccionar la clave con el ratón y darle con el botón derecho a copiar.

**2. En la otra terminal conectada al servidor remoto Debian**

Conéctate al servidor Debian como `admin` si no lo estabas ya.

Cada usuario guarda las claves públicas en el archivo denominado **~/.ssh/authorized_keys**. De hecho podrías ver la clave pública del usuario `admin` correspondiente a la clave privada con la que se conecta con:

```
$ cat /home/admin/.ssh/authorized_keys
```

Pero ahora necesitamos que `profe` también tenga su clave pública, así que desde el usuario `admin` cambiaremos al usuario `profe`
